...
class MainScreenComposeTest {
    ...
    fun each_row_in_LazyColumn_has_symbol_name_current_price_price_change_indicator() {
        val firstRow = composeTestRule.onNodeWithTag("LazyColumn").onChildAt(0)
        firstRow.onChildAt(0).assertTextContains("AAPL")
        firstRow.onChildAt(1).assertTextContains("$300.00")
        firstRow.onChildAt(2).assertContentDescriptionContains("Neutral")

    }

    @Test
    fun items_in_LazyColumn_are_sorted_by_price_descending() {
        composeTestRule.onNodeWithTag("LazyColumn").onChildAt(0).onChildAt(0)
            .assertTextContains("AAPL")
        composeTestRule.onNodeWithTag("LazyColumn").onChildAt(1).onChildAt(0)
            .assertTextContains("MSFT")
        composeTestRule.onNodeWithTag("LazyColumn").onChildAt(2).onChildAt(0)
    ...
    @Test
    fun tapping_to_a_row_opens_the_symbol_details_page() {
        composeTestRule.onNodeWithTag("LazyColumn").onChildAt(0).performClick()
        composeTestRule.onNodeWithTag("SymbolDetailsTitle").assertTextContains("AAPL")
    }
}